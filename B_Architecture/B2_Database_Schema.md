...
Hệ thống thảo luận bao gồm phòng thảo luận chung (Discussion Room) chia thành các chủ đề (Topics) và các bài viết (Posts), bên cạnh đó các bài nộp bài tập (`submissions`) cũng hoạt động như các threads thảo luận riêng biệt.

### `discussion_topics` (Chủ đề Thảo luận)

| Column Name   | Type          | Constraints        | Description                        |
| :------------ | :------------ | :----------------- | :--------------------------------- |
| `id`          | `uuid`        | PK                 |                                    |
| `name`        | `text`        | NOT NULL           | Tên chủ đề (VD: Light Support)     |
| `description` | `text`        |                    | Mô tả chi tiết chủ đề              |
| `created_by`  | `uuid`        | FK (`profiles.id`) | Người tạo chủ đề (Chỉ Admin)       |
| `created_at`  | `timestamptz` | DEFAULT `now()`    |                                    |

### `discussion_posts` (Bài thảo luận)

| Column Name     | Type          | Constraints                  | Description                                  |
| :-------------- | :------------ | :--------------------------- | :------------------------------------------- |
| `id`            | `uuid`        | PK                           |                                              |
| `topic_id`      | `uuid`        | FK (`discussion_topics.id`)  | Thuộc chủ đề nào                             |
| `author_id`     | `uuid`        | FK (`profiles.id`)           | Người viết bài                               |
| `title`         | `text`        | NOT NULL                     | Tiêu đề bài thảo luận                        |
| `content`       | `text`        | NOT NULL                     | Nội dung thảo luận (Rich-text)               |
| `tags`          | `text[]`      |                              | Các nhãn phân loại (Ví dụ: tên Bài tập)      |
| `upvotes_count` | `int`         | DEFAULT 0                    | Tổng số lượt thích                           |
| `upvoted_by`    | `text[]`      |                              | Danh sách ID học viên đã thích               |
| `created_at`    | `timestamptz` | DEFAULT `now()`              |                                              |
...
| :-------------- | :------------ | :-------------------- | :------------------------ |
| `id`            | `uuid`        | PK                    |                           |
| `submission_id` | `uuid`        | FK (`submissions.id`) | Bình luận vào bài nộp nào (hoặc post thảo luận nếu mở rộng) |
